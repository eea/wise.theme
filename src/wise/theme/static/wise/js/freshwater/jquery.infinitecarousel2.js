		...
		var options = $.extend(defaults, options);
			...
			if(o.displayThumbnails)
			{
				...
				for(i=0;i<=numImages-1;i++)
				{
					...
					$('#thumbs'+randID).append('<div class="thumb" id="thumb'+randID+'_'+(i+1)+'" style="position:relative;cursor:pointer;background-image:url('+thumb+');display:inline;float:left;width:'+o.thumbnailWidth+';height:'+o.thumbnailHeight+';line-height:'+o.thumbnailHeight+';padding:0;overflow:hidden;text-align:center;border:2px solid #ccc;margin-right:4px;font-size:'+o.thumbnailFontSize+';font-family:Arial;color:#000;text-shadow:0 0 3px #fff">'+(i+1)+'</div>');
					if(i<=o.inView) {
					    $('#thumb'+randID+'_'+i).css({'border-color':'#ff0000'});
					    $('#thumb'+randID+'_'+i).attr('class', 'thumb_focus');
					}
					unviewable.push(i+1);
				}
			...
			function preMove()
			{
				...
				if(o.displayThumbnails) for(i=1;i<=numImages;i++) 
				    { 
				        ...
				        $('#thumb'+randID+'_'+i).attr('class', 'thumb');
				    }
			}
			...
			function postMove()
			{
				...
				if(o.displayThumbnails) for(i=0;i<viewable.length;i++) {
				    ...
				    $('#thumb'+randID+'_'+viewable[i]).attr('class', 'thumb_focus');
				}
